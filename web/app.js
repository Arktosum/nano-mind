import { renderAttentionAtlasLesson } from './lessons/attention.js';
import { renderBlocksLesson } from './lessons/blocks.js';
import { renderConcatLesson } from './lessons/concat.js';
import { renderEmbeddingLesson } from './lessons/embedding.js';
import { renderFfwdLesson } from './lessons/ffwd.js';
import { renderFlashAttentionLesson } from './lessons/flash_attention.js';
import { renderGenerateLesson } from './lessons/generate.js';
import { renderGQALesson } from './lessons/gqa.js';
import { renderKvCacheLesson } from './lessons/kv_cache.js';
import { renderLayerNormLesson } from './lessons/layernorm.js';
import { renderLn2Lesson } from './lessons/ln2.js';
import { renderMlaLesson } from './lessons/mla.js';
import { renderMoeLesson } from './lessons/moe.js';
import { renderOutputLesson } from './lessons/output.js';
import { renderPagedAttentionLesson } from './lessons/paged_attention.js';
import { renderPositionSumLesson } from './lessons/position.js';
import { renderProfilingLesson } from './lessons/profiling.js';
import { renderProjLesson } from './lessons/proj.js';
import { renderQKVLesson } from './lessons/qkv.js';
import { renderRecapLesson } from './lessons/recap.js';
import { renderResid1Lesson } from './lessons/resid1.js';
import { renderResid2Lesson } from './lessons/resid2.js';
import { renderRingAttentionLesson } from './lessons/ring_attention.js';
import { renderScoresLesson } from './lessons/scores.js';
import { renderSoftmaxLesson } from './lessons/softmax.js';
import { renderSpeculativeDecodingLesson } from './lessons/speculative_decoding.js';
import { renderValuesLesson } from './lessons/values.js';
...
const LESSONS = [
  ...
  },
  {
    ...
    short: "generate",
    eq: `<div class="eq">P(x<sub>i</sub> | x<sub>1…t</sub>) = <span class="frac"><span class="num"><i>e</i><sup> z<sub>i</sub> / T </sup></span><span class="den">∑<sub>j</sub> <i>e</i><sup> z<sub>j</sub> / T </sup></span></span></div>`,
    why:
      ...
      "<p>But how do we pick that next character? We use <b>Temperature Scaling</b> on the logits before passing them to the Softmax function. Here is what each term means:</p>" +
      "<ul>" +
      "<li><b>P(x<sub>i</sub> | x<sub>1…t</sub>)</b>: The final probability of picking character <i>i</i>, given the past context.</li>" +
      "<li><b>z<sub>i</sub></b>: The raw score (logit) for character <i>i</i> generated by the model's output layer.</li>" +
      "<li><b>T</b>: The <b>Temperature</b> parameter.</li>" +
      "<li><b>∑<sub>j</sub></b>: The sum over all possible characters <i>j</i> in the vocabulary, ensuring probabilities add up perfectly to 1.</li>" +
      "</ul>" +
      "<p><b>How Temperature works:</b><br>" +
      "• <b>T = 1.0:</b> Standard softmax. The model uses its learned confidence.<br>" +
      "• <b>T &lt; 1.0 (e.g. 0.5):</b> Dividing by a small T makes the logits larger. The probability distribution sharpens, making the model more deterministic and confident (great for coding).<br>" +
      "• <b>T &gt; 1.0 (e.g. 1.5):</b> Dividing by a large T flattens the logits. Lower-probability characters get a boost, making the output more chaotic and creative.<br>" +
      "• <b>T → 0:</b> The formula collapses into <b>argmax</b> (greedy decoding), where the highest logit takes 100% of the probability.</p>",
    render: (host) => renderGenerateLesson(host, ctx),
  },
  {
    crumb: "generate / profiling — inference time bottlenecks",
    title: "Inference Profiler",
    short: "profiling",
    eq: `<div class="eq">Where does the engine spend its time?</div>`,
    why:
      "<p>Now that we've run the full forward pass, let's look at exactly how long each operation took to execute.</p>" +
      "<p>In massive LLMs running on GPU clusters, the bottlenecks are often <b>Memory Bandwidth</b> (moving massive weight matrices into the compute cores) rather than just raw FLOPs (math). However, even in our tiny browser-based model, you can see how the cost of Self-Attention and the Feed Forward Network compare to simple lookups.</p>" +
      "<p>This page profiles the exact JavaScript execution time of each major block in your browser right now.</p>",
    render: (host) => renderProfilingLesson(host, ctx),
  },
  {
  ...
  {
    crumb: "advanced / sparsity — mixture of experts",
    title: "Mixture of Experts",
    short: "moe",
    eq: `<div class="eq">Scale parameters up, keep FLOPs down</div>`,
    why: "<p>A lesson on the mechanics of sparse routing and MoE architectures.</p>",
    render: (host) => renderMoeLesson(host, ctx),
  },
  {
    crumb: "advanced / hardware — flash attention",
    title: "FlashAttention",
    short: "flash_attention",
    eq: `<div class="eq">Beat the Memory Wall</div>`,
    why: "<p>A lesson on IO-Awareness, SRAM vs HBM, and tiling attention computation to drastically speed up training and inference.</p>",
    render: (host) => renderFlashAttentionLesson(host, ctx),
  },
  {
    crumb: "advanced / decoding — speculative decoding",
    title: "Speculative Decoding",
    short: "speculative_decoding",
    eq: `<div class="eq">Verify in parallel, generate for free</div>`,
    why: "<p>A lesson on using tiny draft models to hallucinate tokens, and massive target models to verify them all at once.</p>",
    render: (host) => renderSpeculativeDecodingLesson(host, ctx),
  },
  {
    crumb: "advanced / memory — grouped-query attention",
    title: "Grouped-Query Attention",
    short: "gqa",
    eq: `<div class="eq">Shrink the KV Cache</div>`,
    why: "<p>A lesson on sharing Key and Value heads across multiple Query heads to drastically reduce the size of the KV cache.</p>",
    render: (host) => renderGQALesson(host, ctx),
  },
  {
    crumb: "advanced / hardware — paged attention",
    title: "PagedAttention",
    short: "paged_attention",
    eq: `<div class="eq">Virtual memory for the KV Cache</div>`,
    why: "<p>A lesson on eliminating memory fragmentation by breaking the KV cache into small pages mapped to physical blocks in VRAM.</p>",
    render: (host) => renderPagedAttentionLesson(host, ctx),
  },
  {
...
const PHASES = [
  ...
  { label: "Generation", steps: [17, 18] },
  { label: "Advanced", steps: [19, 20, 21, 22, 23, 24, 25, 26] },
];
